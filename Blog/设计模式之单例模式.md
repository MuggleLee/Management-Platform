...
```
由方法的注释就知道这个方法是反序列化的实现方法。首先一开始是一些if判断，先跳过。接下来是switch...case...的判断，因为我传进去的是Object对象，所以重点看return checkResolve(readOrdinaryObject(unshared));这一行。
```java
...
由于readOrdinaryObject方法返回的是Object对象，所以根据源码，Object对象的声明就在obj = desc.isInstantiable() ? desc.newInstance() : null;这一行，我是不是离真相越来越近了呢？接下来继续看isInstantiable源码。
```java
    /**
