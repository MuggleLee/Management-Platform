...
```java
1. true
2. java.lang.NoSuchMethodException: SingletonPattern.EnumSingleton.<init>()
3. 	at java.lang.Class.getConstructor0(Class.java:3082)
4. 	at java.lang.Class.getDeclaredConstructor(Class.java:2178)
5. 	at SingletonPattern.SerializableSingletonTest.main(SerializableSingletonTest.java:32)
6. java.lang.IllegalArgumentException: Cannot reflectively create enum objects
7. 	at java.lang.reflect.Constructor.newInstance(Constructor.java:417)
8. 	at SingletonPattern.SerializableSingletonTest.main(SerializableSingletonTest.java:45)
```
首先看下例子1，输出结果是true，可以证明，序列化反序列化对枚举单例没影响。
接下来看例子2，输出结果报错显示java.lang.NoSuchMethodException，这是因为，枚举类实际上是会继承抽象类Enum，而Enum类只有一个有参构造器，所以通过constructor.newInstance()调用无参构造器是错误的。
那例子3是调用有参构造器，为什么还会报错呢？看报错信息已经很清晰了，Cannot reflectively create enum objects，不能反射创建枚举对象。

结论：
设计单例模式尽量选择用枚举的方式，代码量不大而且能保证线程安全、序列化不受影响、反射不会